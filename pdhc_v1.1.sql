...
ttitle center 'PDHC1.0 -  A Quick Health Check from RENAPS' skip 2
-- btitle center '<span style="background-color:#38761d;color:#ffffff;border:1px solid black;">PART - 1</span>'
set markup html on spool on entmap off -
    head '<style>body { background-color: #f2f2f2; font-family: monospace; }</style>'
set markup html on spool on entmap off

COLUMN db_name NEW_VALUE db_name NOPRINT;
COLUMN timestamp NEW_VALUE timestamp NOPRINT;

-- SELECT LOWER(name) db_name FROM v$database;
SELECT LOWER(SYS_CONTEXT('USERENV', 'CON_NAME')) AS db_name FROM dual;
SELECT TO_CHAR(SYSDATE, 'YYYYMMDD_HH24MISS') timestamp FROM dual;

SPOOL &db_name._healthcheck_&timestamp..html

PROMPT <h1 class="banner">RENAPS - Database Health Checker</h1>
-- RENAPS ASCII Banner using <pre> for fixed formatting
PROMPT <pre style="font-weight: bold; color: #005792; font-size: 14px;">
PROMPT </pre>

COLUMN current_instance NEW_VALUE current_instance NOPRINT;
SELECT SYS_CONTEXT('USERENV', 'CON_NAME') current_instance FROM dual;
alter session set nls_date_format = 'DD-MON-YYYY HH24:MI:SS';
set linesize 400 pagesize 400
SET TERMOUT ON;


PROMPT <h2>Script Details</h2>
PROMPT <ul>
PROMPT   <li><strong>Script:</strong> Renaps Quick HealthCheck Script (pdhc.sql)</li>
PROMPT   <li><strong>Author:</strong> Prashant Dixit</li>
PROMPT   <li><strong>Category:</strong> General Monitoring, Admin, Performance Management and Issue Identification</li>
PROMPT   <li><strong>Version:</strong> V1.0 (Date: 20-05-2025)</li>
PROMPT </ul>
...
--PROMPT +------------------------------------------------------------------------+
PROMPT <div style="font-weight: bold; font-size: 16px; border: 2px solid #005792; padding: 10px; background-color: #e9f1f7;">
PROMPT Report : <span style="color: #000;">Database under Observation</span><br>
PROMPT Instance : <span style="color: #000;">&current_instance</span>
PROMPT </div>
...
rac AS (
  SELECT COUNT(*) instances,
         CASE COUNT(*) WHEN 1 THEN 'Single-instance' ELSE COUNT(*) || '-node RAC cluster' END db_type
  FROM gv$instance
),
...
pdbs AS (
  SELECT COUNT(*) pdb_count,
         LISTAGG(pdb_name, ', ') WITHIN GROUP (ORDER BY pdb_name) pdb_list
  FROM cdb_pdbs
)
SELECT 'Database name:' system_item, db.name system_value FROM db
UNION ALL
...
SELECT 'Database memory:',
  ...
  FROM mem, sga, pga
UNION ALL
SELECT 'Number of PDBs:', TO_CHAR(pdbs.pdb_count) || ' PDB(s)' FROM pdbs
UNION ALL
SELECT 'PDB Names:', pdbs.pdb_list FROM pdbs
UNION ALL
SELECT 'Hardware:', CASE WHEN cell.cnt > 0 THEN 'Engineered System with ' || cell.cnt || ' storage servers' ELSE 'Unknown' END FROM cell
UNION ALL
...
SELECT
  TO_CHAR(A.ORIGINATING_TIMESTAMP, 'DD.MM.YYYY HH24:MI:SS') AS MESSAGE_TIME,
  A.MESSAGE_TEXT,
  A.HOST_ID
FROM
  V$DIAG_ALERT_EXT A
WHERE
  A.ORIGINATING_TIMESTAMP > SYSDATE - 1
  AND A.COMPONENT_ID = 'rdbms'
  AND A.MESSAGE_TEXT LIKE '%ORA-%'
ORDER BY
  A.ORIGINATING_TIMESTAMP DESC;
...
PROMPT +##############################################################################################################################################
PROMPT +##############################################################################################################################################
ttitle center 'PDHC1.0 -  A Quick Health Check --- PART 2' skip 2
...
PROMPT +------------------------------------------------------------------------+
PROMPT | Feature Usage Stats on the database - Licensing Information            |
PROMPT | Instance : &current_instance                                           |
PROMPT +------------------------------------------------------------------------|

COLUMN feature_name       FORMAT A60 HEADING 'Feature Name'
COLUMN usage_status       FORMAT A25 HEADING 'Usage Status'
COLUMN last_sample_date   FORMAT A20 HEADING 'Last Sample'
COLUMN first_usage        FORMAT A20 HEADING 'First Usage'
COLUMN last_usage         FORMAT A20 HEADING 'Last Usage'

PROMPT Feature Usage Report (Filtered: Current and Historical Usage Only)

SELECT
    NAME AS feature_name,
    CASE
        WHEN CURRENTLY_USED = 'TRUE' THEN 'CURRENT_USAGE'
        WHEN DETECTED_USAGES > 0 THEN
            CASE
                WHEN FIRST_USAGE_DATE IS NOT NULL AND LAST_USAGE_DATE IS NOT NULL THEN 'PAST_OR_CURRENT_USAGE'
                ELSE 'PAST_USAGE'
            END
    END AS usage_status,
    LAST_SAMPLE_DATE,
    FIRST_USAGE_DATE AS first_usage,
    LAST_USAGE_DATE AS last_usage
FROM
